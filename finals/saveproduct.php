...
if(isset($_POST['txtpname'])){
    ...
    $id = $_POST['txtid'];
    $qty = htmlspecialchars($_POST['txtqty']);
    $desc = htmlspecialchars($_POST['txtdesc']);
    
    try {
        if($id == 0){
            $sql="INSERT INTO products(pname, descrip, qty, category) VALUES(?, ?, ?, ?)";
            $data = array($pname, $desc, $qty, $category);
        }else{
                $sql="UPDATE products SET pname = ?, descrip = ?, qty = ?, category = ? WHERE md5(productID)  = ?";
                $data = array($pname, $desc, $qty, $category, $id);
        }
